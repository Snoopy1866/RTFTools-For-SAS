...
%macro compare_rtf_with_dataset(rtf,
    ...
    proc sql noprint;
        ...
        select count(*) into : is_readrtf_loaded   from DICTIONARY.CATALOGS where libname = "WORK" and memname = "SASMACR" and objname = "READ_RTF";
    quit;
    ...
    %if not &is_readrtf_loaded %then %do;
        %put ERROR: 前置依赖缺失，请先加载宏程序 %nrstr(%%)read_rtf。;
        %let is_dependency_loaded = 0;
    %end;
    ...
    %if %upcase(&ignore_half_or_full_width) = TRUE %then %do;
        ...
        data _tmp_dataset_char_ver;
            set _tmp_dataset_char_ver;
            %do i = 1 %to &dataset_col_n;
                &&dataset_col_&i = ktranslate(&&dataset_col_&i, %unquote(%superq(half_or_full_width_translation)));
            %end;
        run;
        ...
        data _tmp_rtf;
            set _tmp_rtf;
            %do i = 1 %to &rtf_col_n;
                &&rtf_col_&i = ktranslate(&&rtf_col_&i, %unquote(%superq(half_or_full_width_translation)));
            %end;
        run;
    %end;
    ...
    %put NOTE: 宏 compare_rtf_with_dataset 已结束运行！;
%mend;
