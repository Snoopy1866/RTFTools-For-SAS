...
%macro compare_rtf(base,
    ...
    %if %upcase(&ignore_cell_style) = TRUE %then %do;
        %let reg_cellstyle_expr = %bquote(/^(?:\\clbrdr[tblr]\\brdrs\\brdrw\d+\\brdrcf\d+)*\\cltxlrtb\\clvertal[tcb](?:\\clcbpat\d+)?(?:\\clpadt\d+\\clpadft\d+\\clpadr\d+\\clpadfr\d+)?\\cellx\d+$/o);
...
/* --- caller: exercise compare_rtf() on two small, self-contained RTF-shaped
   fixtures differing in one data cell (20/22 vs 20/23 subjects). The
   fixtures are written to disk from inline DATALINES so the bundle needs
   no external input/ file -- compare_rtf() itself still reads them back
   from disk via infile, exactly as the macro is written. --- */
data _null_;
    infile datalines truncover;
    file "base.rtf" lrecl=200;
    input;
    put _infile_;
datalines;
{\rtf1\ansi
\outlinelevel0{Table 14.1.1 Subject Disposition}
\pard\plain\intbl\sb0\sa0\ql\f0\fs20\cf0{Placebo\cell}
\pard\plain\intbl\sb0\sa0\ql\f0\fs20\cf0{20\cell}
\pard\plain\intbl\sb0\sa0\ql\f0\fs20\cf0{Active\cell}
\pard\plain\intbl\sb0\sa0\ql\f0\fs20\cf0{22\cell}
}
run;

data _null_;
    infile datalines truncover;
    file "compare.rtf" lrecl=200;
    input;
    put _infile_;
datalines;
{\rtf1\ansi
...
\pard\plain\intbl\sb0\sa0\ql\f0\fs20\cf0{20\cell}
\pard\plain\intbl\sb0\sa0\ql\f0\fs20\cf0{Active\cell}
\pard\plain\intbl\sb0\sa0\ql\f0\fs20\cf0{23\cell}
}
run;

filename basertf "base.rtf";
filename cmprtf "compare.rtf";
%compare_rtf(base = basertf, compare = cmprtf, outdata = rtf_diff);

proc print data = rtf_diff label noobs;
run;
